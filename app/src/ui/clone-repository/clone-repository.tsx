...
> {
  ...
  public render() {
    ...
    return (
      <Dialog
        ...
        title={platformT('cloneRepository.title')}
        onSubmit={this.clone}
  ...
  private renderSignIn(tab: CloneRepositoryTab) {
    const signInTitle = platformT('cloneRepository.signInTitle')
    switch (tab) {
      case CloneRepositoryTab.DotCom:
        return (
          <CallToAction actionTitle={signInTitle} onAction={this.signInDotCom}>
            <div>{t('cloneRepository.signInDotCom')}</div>
          </CallToAction>
        )
      case CloneRepositoryTab.Enterprise:
        return (
          ...
          >
            <div>{t('cloneRepository.signInEnterprise')}</div>
          </CallToAction>
        )
      case CloneRepositoryTab.Generic:
        return null
  ...
  private onChooseWithSaveDialog = async (): Promise<string | undefined> => {
    ...
    const path = await showSaveDialog({
      buttonLabel: t('cloneRepository.select'),
      nameFieldLabel: t('cloneRepository.cloneAs'),
      showsTagField: false,
  ...
  private async validateEmptyFolder(
    ...
    if (path === null) {
      return new Error(t('cloneRepository.pathReadError'))
    }
    ...
    try {
      ...
      if (directoryFiles.length === 0) {
        ...
        return new Error(t('cloneRepository.filesInFolder'))
      }
    } catch (error) {
      if (error.code === 'ENOTDIR') {
        // path refers to a file or other file system entry
        return new Error(t('cloneRepository.fileWithName'))
      }
  ...
  private clone = async () => {
    ...
    if (path == null) {
      const error = new Error(t('cloneRepository.dirCreateError'))
    ...
    if (!cloneInfo) {
      const error = new Error(t('cloneRepository.repoNotFound'))
      this.setState({ loading: false })
      this.setSelectedTabState({ error })
      return
    }
